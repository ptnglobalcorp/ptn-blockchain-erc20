var CepToken = artifacts.require('PtnCoinToken')

contract('PtnCoinToken', function (accounts) {

  var tokenInstance;
  var decimalPlaces = 18;
  var initialSupply = 1000 * (10 ** decimalPlaces);

  // it('initializes the contract with the correct values', function () {
  //   return CepToken.deployed().then(function (instance) {
  //     tokenInstance = instance;
  //     return tokenInstance.name();
  //   }).then(function (name) {
  //     assert.equal(name, 'CEP Token', 'has the correct name');
  //     return tokenInstance.symbol();
  //   }).then(function (symbol) {
  //     assert.equal(symbol, 'CEP', 'has the correct symbol');
  //     return tokenInstance.standard();
  //   }).then(function (standard) {
  //     assert.equal(standard, 'Cep Token v1.0', 'has the correct standard');
  //   });
  // })
  ...
  //     assert.equal(totalSupply, initialSupply, 'sets the total supply to 1000 * 10**18');
  ...
  //   }).then(function (success) {
  //     assert.equal(success, true, 'it returns true');
  //     return tokenInstance.approve(accounts[1], 50);
  //   }).then(function (receipt) {
  //     assert.equal(receipt.logs.length, 1, 'triggers one event');
  ...
  //     assert.equal(allowance.toNumber(), 50, 'stores the allowance for delegated transfer');
  //   });
  // });
